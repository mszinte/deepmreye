...
for subject in subjects:    
    ...
    for run in range(num_run): 
         # Identify mask and label files
            ...
            label_filename = f"{subject}_run_0{run + 1}_simulated_labels.npy"

            mask_path = os.path.join(mask_dir, subject, mask_filename)
            label_path = os.path.join(model_dir, "sim_labels", label_filename)
    ...
    preprocess.save_data(participant=f"{subject}_DeepMReyeCalib_label_sim",
...
for generators in cv_generators:    
    ...
    # pre-load the model
    (preload_model, preload_model_inference) = train.train_model(dataset="DeepMReyeCalib_sim", 
    ...
    with warnings.catch_warnings():
        ...
        (model, model_inference) = train.train_model(dataset="DeepMReyeCalib_sim", 
                                                        generators=generators, 
                                                        opts=opts, 
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        (cv_evaluation, cv_scores) = train.evaluate_model(dataset="DeepMReyeCalib_sim",
        ...
        np.save(f"{pred_dir}/evaluation_dict_calib_sim.npy",evaluation)
# Sava data      
np.save(f"{pred_dir}/evaluation_dict_calib_sim.npy",evaluation)
   
np.save(f"{pred_dir}/scores_dict_calib_sim.npy",scores)
