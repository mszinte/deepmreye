...
2. python train_calib_closed_vs_open.py [main directory] [project name] [group]
-----------------------------------------------------------------------------------------
Exemple:
...
pp_dir = f"{main_dir}/pp_data_closed_vs_open_calib"
...
for subject in subjects:    
    ...
    preprocess.save_data(participant=f"{subject}_DeepMReyeCalib_label_closed",
...
for cv_idx, generators in enumerate(cv_generators):    
    ...
    # pre-load the model
    (preload_model, preload_model_inference) = train.train_model(dataset="closed_DeepMReyeCalib", 
    ...
    with warnings.catch_warnings():
        ...
        (model, model_inference) = train.train_model(dataset="closed_DeepMReyeCalib", 
                                                        generators=generators, 
                                                        opts=opts, 
...
np.save(f"{pred_dir}/evaluation_dict_gaze_closed_vs_open_calib.npy", evaluation)
print(f"All predictions saved to {pred_dir}/evaluation_dict_gaze_closed_vs_open_calib.npy")
