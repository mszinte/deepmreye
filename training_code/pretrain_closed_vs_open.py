...
2. pretrain_closed_vs_open.py [main directory] [project name] [group]
-----------------------------------------------------------------------------------------
Exemple:
cd ~/projects/deepmreye/training_code
python pretrain_closed_vs_open.py /scratch/mszinte/data deepmreye 327 
-----------------------------------------------------------------------------------------
"""
...
model_file = f"{model_dir}modelinference_closed_DeepMReyeCalib.h5" #predicting closed data with calib fine tuned weights
#f"{model_dir}dataset6_openclosed.h5"
pp_dir = f"{main_dir}/pp_data_closed_vs_open_ftcalib/"
...
for subject in subjects:    
    ...
    preprocess.save_data(participant=f"{subject}_DeepMReyeClosed_no_label",
...
(model, model_inference) = train.train_model(dataset="DeepMRyeClosed_ftcalib",
...
np.save(f"{pred_dir}/evaluation_dict_closed_vs_open_ftcalib.npy", evaluation)
np.save(f"{pred_dir}/scores_dict_closed_vs_open_ftcalib.npy", scores)
...
np.save(f"{pred_dir}/ftcalib_eyes_closed_predictions.npy", eyes_closed_preds)
