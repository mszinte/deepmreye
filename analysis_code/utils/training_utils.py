...
def _compute_ee(eye_data, pred_x, pred_y):
    n = min(len(eye_data), len(pred_x), len(pred_y))
    ee = np.sqrt(
        (eye_data[:n, 0] - pred_x[:n])**2 +
        (eye_data[:n, 1] - pred_y[:n])**2
    )
    return ee, n

def compute_ee_by_ecc_sector(
    eye_data: np.ndarray,
    pred_x: np.ndarray,
    pred_y: np.ndarray,
    ecc_bins: list  = [0, 2.25, 6.75, 10.0],
    ecc_labels: list = ["foveal", "parafoveal", "peripheral"],
    n_sectors: int  = 4,
    min_ecc: float  = 1.0,
) -> pd.DataFrame:
    """
    Cross eccentricity rings with directional sectors.
    Foveal ring gets a single 'center' entry (direction undefined).
    """
    ee, n   = _compute_ee(eye_data, pred_x, pred_y)
    ecc     = np.linalg.norm(eye_data[:n], axis=1)
    angle   = np.degrees(np.arctan2(eye_data[:n, 1], eye_data[:n, 0])) % 360
    bin_idx = (np.digitize(ecc, ecc_bins) - 1).clip(0, len(ecc_labels) - 1)

    sector_width = 360 / n_sectors
    sector_idx = ((angle + sector_width / 2) % 360 / sector_width).astype(int) % n_sectors
    sector_names = ["right", "top", "left", "bottom"] if n_sectors == 4 else \
                   ["right", "top-right", "top", "top-left",
                    "left", "bottom-left", "bottom", "bottom-right"]

    records = []

    # center bin: no sector split
    mask = bin_idx == 0
    records.append({
        "ecc_bin" : ecc_labels[0],
        "sector"  : "center",
        "mean_ee" : np.mean(ee[mask]) if mask.sum() > 0 else np.nan,
        "n_samples": int(mask.sum()),
    })

    # outer bins: split by sector
    for b in range(1, len(ecc_labels)):
        for s in range(n_sectors):
            mask = (bin_idx == b) & (sector_idx == s) & (ecc >= min_ecc)
            records.append({
                "ecc_bin"  : ecc_labels[b],
...
def compute_corr_by_ecc_sector(
    eye_data: np.ndarray,
    pred_x: np.ndarray,
    pred_y: np.ndarray,
    ecc_bins: list   = [0, 3.375, 6.75, 10.125, 12.75],
    ecc_labels: list = ["foveal", "parafoveal", "peripheral", "far peripheral"],
    n_sectors: int   = 4,
    min_ecc: float   = 1.0,
) -> pd.DataFrame:
    """
    Compute Pearson correlation between eye tracker and model predictions
    per eccentricity ring × directional sector.
    Correlation is computed on the concatenated X and Y dimensions.
    Foveal ring gets a single 'center' entry (direction undefined).
    Returns NaN for cells with fewer than 3 samples (correlation undefined).
    """
    n       = min(len(eye_data), len(pred_x), len(pred_y))
    ecc     = np.linalg.norm(eye_data[:n], axis=1)
    angle   = np.degrees(np.arctan2(eye_data[:n, 1], eye_data[:n, 0])) % 360
    ...
    pred = np.stack([pred_x[:n], pred_y[:n]], axis=1)  # (N, 2)

    def _corr(eye_subset, pred_subset):
        """Pearson r on flattened X+Y, returns NaN if too few samples."""
        if len(eye_subset) < 3:
            return np.nan
        eye_flat  = eye_subset.flatten()
        pred_flat = pred_subset.flatten()
        # remove pairs where either is NaN
        valid = ~(np.isnan(eye_flat) | np.isnan(pred_flat))
        if valid.sum() < 3:
            return np.nan
        return float(np.corrcoef(eye_flat[valid], pred_flat[valid])[0, 1])

    records = []

    # foveal — no sector split
    mask = bin_idx == 0
    records.append({
        "ecc_bin"  : ecc_labels[0],
        "sector"   : "center",
        "corr"     : _corr(eye_data[:n][mask], pred[mask]),
        "n_samples": int(mask.sum()),
    })
